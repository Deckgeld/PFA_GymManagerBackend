...
if (Environment.GetEnvironmentVariable("PORT") is not null and string environmentPort   //Verifica si no es nulo asigna su valor a la variable environmentPort
    && string.IsNullOrEmpty(environmentPort) == false     // Asegurar que la variable tenga un valor (no nulo ni vac�o)
    && int.TryParse(environmentPort, out int port))      // Intentar convertir el valor a un entero
{
    builder.WebHost.ConfigureKestrel(o => o.ListenAnyIP(port)); // Configurar Kestrel para escuchar en cualquier direcci�n IP y el puerto analizado
}
